...
void initVarTable(){
	...
	FuncDef *readfunc = malloc(sizeof(FuncDef));
	strcpy(readfunc->name, "read");
	readfunc->rtn = malloc(sizeof(Type));
	readfunc->rtn->kind = BASIC;
	readfunc->rtn->u.basic = INTTYPE;
	readfunc->param = NULL;
	readfunc->is_define = true;
	readfunc->line = 0;
	insertFunc(readfunc);

	// define write() function
	FuncDef *writefunc = malloc(sizeof(FuncDef));
	strcpy(writefunc->name, "write");
	writefunc->rtn = malloc(sizeof(Type));
	writefunc->rtn->kind = BASIC;
	...
	writefunc->is_define = true;
	writefunc->line = 0;
	insertFunc(writefunc);
